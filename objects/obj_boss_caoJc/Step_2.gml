...
if action==2.3 {
	if time==0 
	|| !instance_exists(obj_boss_caoJc_bullet_missile_b) {
		action=2.4
	}
}
if action==2.4 {
...
if action==3.1 {
	if ry<=-160 {
		...
		if(global.boss_hp<=hp_up/2) 
			arm_block.forearm.craw.blockReady(obj_boss_caoJc_block_bomb01)
		else
			arm_block.forearm.craw.blockReady(obj_boss_caoJc_block_bomb02)
		vspeed=2
		action=3.2
	}
}
