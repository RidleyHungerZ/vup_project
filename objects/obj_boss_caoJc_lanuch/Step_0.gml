...
if action=11 && time==0 {
	if missile_index>=array_length(missile_pos) {
		...
		with instance_create_depth(x+pnt.x*image_xscale, y+pnt.y*image_yscale, depth-1, obj_boss_caoJc_bullet_missile_s) {
...
if action=21 && time==0 {
	with instance_create_depth(x+0*image_xscale, y+6*image_yscale, depth-1, obj_boss_caoJc_bullet_missile_b) {
		direction=90-90*other.image_xscale
		image_angle=direction
	}
	scr_sound_play(se_enemy_missile)
